...
1. **Signed APK build + on-device verification — High.** Cannot be done from this environment - no Android SDK, no Godot export templates, and the outbound network proxy explicitly policy-denies both `dl.google.com` and `github.com/godotengine` (confirmed via the proxy's own status endpoint, not a guess). What *could* be done here without any of that has been done and is documented in `docs/EXPORT_SIZE_AUDIT.md` and `docs/layout-captures/`: the export filter's correctness was verified directly against Godot's own `String.match()` (not assumed), current would-ship source weight was measured at 212.4 MB - already at/over the 200 MB target before any texture/audio compression - and desktop-rendered layout checks at 720×1600/1080×2400/1440×3200 on Haven and a Scavenging location found no clipping or overlap. What's left needs the real toolchain: applying the ETC2/audio-compression changes the size audit identifies, a signed build, real-device frame-time/memory measurement, and install/upgrade/pause-resume/persistence testing.
2. **Verified obsolete-placeholder cleanup — Medium.** Remove only assets proven unreachable in an exported build. Defensive error/future fallbacks (listed above) should remain until the export dependency audit above is acted on.

Resolved since the previous version of this report: the Android launcher/adaptive icon is final and integrated (`docs/production-batches/19_android_launcher_identity.md`); manifest reconciliation was the prior update; chain-specific merge VFX, per-residence environment layers, and gameplay-neutral danger presentation (the whole of Batch 3) are done; the export-size audit and desktop layout testing (this update) are as much of Batch 4 as is possible without the Android toolchain itself.
