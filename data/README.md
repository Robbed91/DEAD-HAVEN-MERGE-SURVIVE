...
`items/`, `chains/`, `residences/` and `quests/` were each generated once
by a script run through the Godot binary (see DEVELOPMENT_LOG.md Phase 2
and Phase 3) and the generator was deleted afterward - the `.tres`/`.json`
files here are the real content now; edit them directly (or write a new
one-off script if you need to bulk-regenerate after a schema change).
Every other folder is still just the empty structure until its phase
lands.
