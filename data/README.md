...
| `quests/` | `QuestDefinition` | **Phase 3/6/8/10/11 - partially populated.** 9 residence-repair quests for Hollow Creek Farmhouse (Phase 3) + Noah's personal quest `pq_noah_workbench` (Phase 6, the first `SURVIVOR_PERSONAL`-type quest, not tied to a hotspot) + 8 residence-repair/rescue quests each for Redwater Service Station (Phase 8, `q_rescue_lena`), Greybridge School (Phase 10, `q_rescue_riley`), and Saint Mercy Hospital (Phase 11, `q_rescue_imogen`); other quest types (main story, daily, etc.) land with their respective phases |
| `characters/` | `SurvivorDefinition` | **Phase 6 - populated.** All 6 named survivors, fully written even though only Mara/Noah/Lena/Riley/Imogen have an unlock path yet |
| `residences/` | `ResidenceDefinition`, `ResidenceHotspot` | **Phase 3/8/10/11 - populated.** Hollow Creek Farmhouse (9 hotspots, Phase 3), Redwater Service Station (8 hotspots, Phase 8), Greybridge School (8 hotspots, Phase 10), and Saint Mercy Hospital (8 hotspots, Phase 11); 1 more residence remains unbuilt |
| `dialogue/` | `DialogueEntry` | **Phase 4/8/10/11 - partially populated.** The Chapter 1 intro (3 entries), and one 3-entry rescue scene each for Noah (Ch.2, Phase 4), Lena (Ch.5, Phase 8), Riley (Ch.6, Phase 10), and Imogen (Ch.7, Phase 11) - later chapters' dialogue lands with the phases that unlock them (see DEVELOPMENT_LOG.md) |
...
`dialogue/riley_*.tres`), and Saint Mercy's Phase 11 content
(`residences/saint_mercy_hospital.tres`, its 8 quests, and
`dialogue/imogen_*.tres`) were all hand-written directly rather than
