...
extends Control
## Running-game screenshots of all six Saint Mercy visual milestones.

const OUTPUT_DIR := "res://docs/saint_mercy-captures"
...
func _ready() -> void:
	...
	for state_index in COUNTS.size():
		...
		GameManager.profile.current_chapter_id = "chapter_7_do_no_harm"
		...
		if state_index == 5:
			...
			GameManager.profile.unlocked_survivor_ids.append("imogen_shaw")
		var scene: Control = load("res://scenes/saint_mercy/saint_mercy.tscn").instantiate()
		...
		scene.queue_free()
		await get_tree().process_frame
	print("SAINT_MERCY_CAPTURE_OK states=6")
	get_tree().quit()
