...
extends Node
## Save/reload verification for each major Northgate visual-state threshold.
...
func _ready() -> void:
	...
	for state_index in COUNTS.size():
		...
		if restored != COUNTS[state_index] or DefenceManager.has_survived(EVENT_ID) != (state_index == 5):
			...
			return
	print("SMOKE_NORTHGATE_VISUAL_STATES_OK states=6")
	get_tree().quit(0)

func _fail(message: String) -> void:
	print("SMOKE_NORTHGATE_VISUAL_STATES_FAIL: %s" % message)
	get_tree().quit(1)
