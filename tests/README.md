...
- **smoke_test_scavenge_merge** (2026-08-05 follow-up) - covers the merge challenge that now gates scavenging success (see DEVELOPMENT_LOG.md "scavenging becomes a merge challenge"): `ScavengeMergeState`'s board seeding always contains enough level-1 tiles of the primary chain to reach the target level; `try_merge()` follows the exact same chain+level rule as `BoardState.try_merge()` and spends exactly one move per successful merge; reaching `target_level` sets `is_won()` and running out of moves without it sets `is_lost()`; `ScavengingManager.compute_challenge_params()` grants more moves for a higher-`success_chance` encounter choice than a lower one; `resolve_choice_with_outcome()` grants the exact same rewards/penalty/text as the existing dice-based `resolve_choice()` for a known outcome, so none of the ten missions' authored loot/penalty/text data needed to change. `capture_scavenge_merge.tscn` drives the real `scavenging.tscn` scene (select survivor, send, pick a choice) and captures the resulting live merge grid, not just the isolated state logic.
...
- **`capture_scavenge_merge.gd`/`.tscn`** - same `xvfb-run` pattern, drives
  the real `scavenging.tscn` scene through send -> choose an approach and
  captures the live merge-challenge grid it opens into, confirming the
  wiring works end to end in the actual screen and not only in
  `smoke_test_scavenge_merge`'s isolated state-logic checks.
