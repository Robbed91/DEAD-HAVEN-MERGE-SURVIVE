...
- **smoke_test_character_portraits** - verifies Mara's neutral, concerned, and injured portrait paths are registered in character data and load as final textures; confirms a missing asset degrades to the procedural survivor fallback; and instantiates the merge board to prove its runtime grid remains exactly 7 columns by 9 rows (63 cells).
